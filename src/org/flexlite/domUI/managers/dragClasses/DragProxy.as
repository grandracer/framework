package org.flexlite.domUI.managers.dragClasses
{
	...
	public class DragProxy extends UIComponent
	{
		...
		private function mouseMoveHandler(event:MouseEvent):void
		{
			...
			DragProxy.getObjectsUnderPoint(stageRoot, stagePoint, targets);
		 ...
		 * 获取舞台下有鼠标事件的显示对象
		 */
		private static function getObjectsUnderPoint(dspObject:DisplayObject, point:Point, result:Vector.<DisplayObject>):void
		{
			if (dspObject.visible && (dspObject is Stage || dspObject.hitTestPoint(point.x, point.y, true)))
			{
				var intObject:InteractiveObject = dspObject as InteractiveObject;
				if (intObject != null && intObject.mouseEnabled) result.push(intObject);
				var container:DisplayObjectContainer = dspObject as DisplayObjectContainer;
				if (container != null && container.mouseChildren && container.numChildren > 0)
					for (var i:int = 0, len:int = container.numChildren; i < len; ++i)
						getObjectsUnderPoint(container.getChildAt(i), point, result);
			}
		}
